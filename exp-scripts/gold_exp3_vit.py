...
# ViT typically expects 224x224 images, so we'll resize
...
# Define ViT model using ViTConfig and ViTForImageClassification
...
clean_model = ViTModel(num_classes=100).to(device)
...
def train_and_evaluate(model, train_loader, val_loader, test_loader, optimizer, criterion, device, num_epochs=30, patience=5):
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(best_model_state, '/scratch/sumit.k/models/gold_models/gold_vit_cifar100_exp3.pth')
            patience_counter = 0
    ...
    plt.legend()
    plt.savefig('gold_exp3_vit_poisoned_training_curve.png')
    plt.close()
...
if __name__ == "__main__":
    ...
    # load model
    clean_model.load_state_dict(torch.load('/scratch/sumit.k/models/clean_models/clean_vit_cifar100.pth'))
    ...
    gold_model = ViTModel(num_classes=100).to(device)
    gold_optimizer = optim.AdamW(gold_model.parameters(), lr=1e-4, weight_decay=1e-4)

    print(f"Number of parameters in gold model: {sum(p.numel() for p in gold_model.parameters())}")
    ...
    forget_loader = DataLoader(
        forget_dataset, batch_size=batch_size, shuffle=True, num_workers=4
    )
    retain_loader = DataLoader(
        retain_dataset, batch_size=batch_size, shuffle=True, num_workers=4
    )
    ...
    # load poisoned model
    gold_model.load_state_dict(torch.load('/scratch/sumit.k/models/gold_models/gold_vit_cifar100_exp3.pth'))

    print("Gold Standard Poisoned Evaluation")
    forget_metrics = evaluate(gold_model, forget_loader, criterion, device)
    retain_metrics = evaluate(gold_model, retain_loader, criterion, device)
    test_metrics = evaluate(gold_model, poisoned_test_loader, criterion, device)
    test_forget_metrics = evaluate(gold_model, test_forget_loader, criterion, device)
    test_retain_metrics = evaluate(gold_model, test_retain_loader, criterion, device)
    print(
